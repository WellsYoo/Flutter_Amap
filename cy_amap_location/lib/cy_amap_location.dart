...
export 'cy_amap_location_option.dart';

import 'cy_amap_location_option.dart';
...
//      };
  ...
  static CYAMapLocation fromMap(dynamic map) {
    return new CYAMapLocation(
...
class CYAMapLocationClient {
  static const MethodChannel _channel = const MethodChannel('cy_amap_location');

  static StreamController<CYAMapLocation> _locationUpdateStreamController =
      new StreamController.broadcast();

  /// 定位改变监听
  static Stream<CYAMapLocation> get onLocationUpate => _locationUpdateStreamController.stream;

  /// 直接获取到定位，不必先启用监听
  /// @param needsAddress 是否需要详细地址信息
  static Future<CYAMapLocation> getLocation(bool needsAddress) async {
    final dynamic location = await _channel.invokeMethod('getLocation', needsAddress);
    return CYAMapLocation.fromMap(location);
  }

  /// 启动系统
  /// @param options 启动系统所需选项
  static Future<bool> startup(CYAMapLocationOption option) async {
    _channel.setMethodCallHandler(handler);
    return await _channel.invokeMethod("startup", option.toMap());
  }

  /// 更新选项，如果已经在监听，那么要先停止监听，再调用这个函数
  static Future<bool> updateOption(CYAMapLocationOption option) async {
      ...
      case "updateLocation":
        {
          ...
          _locationUpdateStreamController.add(CYAMapLocation.fromMap(args));
        }
        break;
    }
