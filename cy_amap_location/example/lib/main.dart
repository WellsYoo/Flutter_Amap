...
void main() {
  ...
  if (Platform.isIOS) {
    CYAMapSDK.setAMapKey("c8cc37b89d747572dbb9e11256a50672");
  }
...
class _LocationGetState extends State {
  ...
  void _checkPersmission() async {
    ...
    //   }
    // }
    CYAMapLocation loc = await CYAMapLocationClient.getLocation(true);
    setState(() {
      _loc = loc;
    });
  }
...
String getLocationStr(CYAMapLocation loc) {
  if (loc == null) {
    return "正在定位";
  }
...
class _LocationListenState extends State {
  ...
  void _checkPersmission() async {
    ...
    //   }
    // }
    CYAMapLocationClient.onLocationUpate.listen((CYAMapLocation loc) {
      if (!mounted) return;
      setState(() {
    ...
    CYAMapLocationClient.startLocation();
  }
  ...
  void dispose() {
    //注意这里停止监听
    CYAMapLocationClient.stopLocation();
    super.dispose();
  }
}
...
class _HomeState extends State<Home> {
  ...
  void initState() {
    ...
    CYAMapLocationClient.startup(new CYAMapLocationOption(
